...
const BUILD_LOOP = (CARDS, SPACING) => {
  ...
  gsap.set(CARDS, { xPercent: 5000, opacity: 1, scale: 0 })
...
// Click a card to bring it into view
const GALLERY = document.querySelector('.gallery')
GALLERY.addEventListener('click', e => {
  const CARD = e.target.closest('li.gallery__card')
  // Work out the index.
  if (CARD) {
    SCRUB_TO(CARDS.indexOf(CARD) * SPACING)
  }
})
