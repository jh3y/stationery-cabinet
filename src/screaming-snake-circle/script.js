...
set('.snake__backdrop', { transformOrigin: '50% 50%' })
...
const scream = () => {
  const repeat = random(25, 80)
  const delay = random(4, 10)
  timeline({
    delay,
    onStart: () => {
      BLINK_TL.pause()
    ...
    .to(
      ['.snake__backdrop', '#circleClip'],
      {
        duration: SCREAM_DURATION,
        scale: 1.65,
        '--backdrop-hue': 0,
        '--backdrop-lightness': 60,
      },
      '<'
    )
    .to(
